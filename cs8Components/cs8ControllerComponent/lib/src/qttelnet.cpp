...
const uchar LineModeEOF = 236,  // RFC1184, not implemented
    LineModeSUSP = 237, LineModeABORT = 238;
...
const char SLC = 3;
enum Modes { EDIT = 1, TRAPSIG = 2, MODE_ACK = 4, SOFT_TAB = 8, LIT_ECHO = 16 };
enum SLCs {
  SLC_SYNCH = 1,
  SLC_BRK = 2,
  SLC_IP = 3,
...
public:
  ...
  bool connected, nocheckp, loggedIn;
...
QtTelnetPrivate::QtTelnetPrivate(QtTelnet *parent)
    : q(parent), socket(0), notifier(0), connected(false), nocheckp(false),
      triedlogin(false), triedpass(false), firsttry(true), curauth(0),
      nullauth(false), loggedIn(false), loginp("ogin:\\s*$"),
      passp("assword:\\s*$"), promptp("->") {
...
void QtTelnetPrivate::setSocket(QTcpSocket *s) {
  ...
  if (socket) {
    connect(socket, &QAbstractSocket::connected, this,
            &QtTelnetPrivate::socketConnected);
    connect(socket, &QAbstractSocket::disconnected, this,
            &QtTelnetPrivate::socketConnectionClosed);
    connect(socket, &QIODevice::readyRead, this,
            &QtTelnetPrivate::socketReadyRead);
    connect(socket, SIGNAL(error(QAbstractSocket::SocketError)), this,
            SLOT(socketError(QAbstractSocket::SocketError)));
    connect(socket, &QAbstractSocket::stateChanged, this,
            &QtTelnetPrivate::socketStateChanged);
  }
}

QAbstractSocket::SocketState QtTelnetPrivate::socketState() const {
  return socket->state();
}
...
void QtTelnetPrivate::consume() {
  ...
  qDebug() << __FUNCTION__ << ": consume" << data;
  int currpos = 0;
  int prevpos = -1;
  ;
  /// FIXME consume [yes] on AYT and logged in w/o authentication
...
bool QtTelnetPrivate::isCommand(const uchar c) { return (c == Common::DM); }

bool QtTelnetPrivate::isOperation(const uchar c) {
  return (c == Common::WILL || c == Common::WONT || c == Common::DO ||
          c == Common::DONT);
}
...
void QtTelnetPrivate::parseSubNAWS(const QByteArray &data) { Q_UNUSED(data); }
...
void QtTelnetPrivate::parseSubTT(const QByteArray &data) {
  ...
  const char c1[4] = {Common::IAC, Common::SB, Common::TerminalType,
                      Common::IS};
...
int QtTelnetPrivate::parseIAC(const QByteArray &data) {
  ...
  if (data.size() >= 3 && isOperation(data[1])) { // IAC, Operation, Option
    ...
    if (operation == Common::DONT && option == Common::Authentication) {
      ...
      nullauth = true;
      /// TODO is this OK when controller sends authentication request or not?
      loggedIn = true;
    }
...
int QtTelnetPrivate::parsePlaintext(const QByteArray &data) {
  ...
  if (length == -1) {
    // check if IAC in stream
    length = data.indexOf(Common::IAC);
    if (length == -1) {
      length = data.size();
      consumed = length;
    } else {
  ...
  text = text.trimmed();
  text.remove(QRegExp("^\\[yes\\]$"));
  if (!text.isEmpty() && loggedIn)
    emit q->message(text);
  return consumed;
}
...
void QtTelnetPrivate::sendWindowSize() {
  ...
  const char c[9] = {Common::IAC,  Common::SB,  Common::NAWS,
                     (w & 0x00ff), (w >> 8),    (h & 0x00ff),
                     (h >> 8),     Common::IAC, Common::SE};
...
bool QtTelnetPrivate::allowOption(int /*oper*/, int opt) {
  if (opt == Common::Authentication || opt == Common::SuppressGoAhead ||
      opt == Common::LineMode || opt == Common::Status ||
      opt == Common::Logout || opt == Common::TerminalType)
    return true;
...
void QtTelnetPrivate::socketStateChanged(QAbstractSocket::SocketState state) {
 ...
 */
void QtTelnet::logout() { d->sendCommand(Common::DO, Common::Logout); }
...
/*!
    ...
    \sa setWindowSize()
 */
bool QtTelnet::isValidWindowSize() const { return windowSize().isValid(); }
 ...
 */
void QtTelnet::setSocket(QTcpSocket *socket) { d->setSocket(socket); }
...
/*!
    ...
    \sa setSocket()
 */
QTcpSocket *QtTelnet::socket() const { return d->socket; }
...
QAbstractSocket::SocketState QtTelnet::socketState() const {
  return d->socketState();
}

void QtTelnet::sendAYT() { sendControl(AreYouThere); }
