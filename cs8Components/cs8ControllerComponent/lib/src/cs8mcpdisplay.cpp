...
cs8MCPDisplay::cs8MCPDisplay(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::cs8MCPDisplay),
    m_cursorPos(QPoint(0,0)),
    m_textMode(Normal),
    m_background(QColor(220,220,220)),
    m_foreground(QColor(0,0,0))
{
    ...
    connect(m_flashTimer,&QTimer::timeout,[=] () {m_flash=!m_flash; emit toggle(m_flash);});
...
cs8MCPDisplay::~cs8MCPDisplay()
{
    delete ui;
}

void cs8MCPDisplay::put(const QString &text)
{
    put(m_cursorPos,text);
}
...
void cs8MCPDisplay::put(int x, int y, const QString &text)
{
    put(QPoint(x,y),text);
}
...
void cs8MCPDisplay::cls()
{
    ...
    QString t;
    t.fill(QChar(' '),40);
    foreach (cs8MCPDisplayRow* l, m_displayRows) {
        l->put(0,m_textMode,t);
    }
}
...
cs8MCPDisplayRow::cs8MCPDisplayRow(QWidget *parent):
    ...
    m_text(QString().fill(' ',40)),
    m_mode(QByteArray().fill(cs8MCPDisplay::Normal,40)),
    m_toggle(false)
{
...
void cs8MCPDisplayRow::update()
{
    ...
    /*
     * <html><head/><body><p>Text<span style=" color:#ff5500;">Label </span><span style=" color:#55aa00;">inversed</span></p></body></html>
     * */
    ...
    for (int i=0;i<40;i++)
    {
        if (i==0 || (m_mode[i]!=m_mode[i-1]))
        {
            ...
            case cs8MCPDisplay::Inversed_Blinking:
                fg=m_foreground;
                bg=m_toggle?m_foreground:m_background;
                break;
            }
            if (i!=0)
            {
...
QLabel *cs8MCPDisplayRow::getLabel() const
{
    return m_label;
}

QString &cs8MCPDisplayRow::getText()
{
    return m_text;
}

QByteArray &cs8MCPDisplayRow::getMode()
{
    return m_mode;
}

QColor cs8MCPDisplayRow::getForeground() const
{
    return m_foreground;
}

void cs8MCPDisplayRow::setForeground(const QColor &foreground)
{
    m_foreground = foreground;
}

QColor cs8MCPDisplayRow::getBackground() const
{
    return m_background;
}

void cs8MCPDisplayRow::setBackground(const QColor &background)
{
