...
cs8NetworkScanner::cs8NetworkScanner(QObject *parent) : QAbstractTableModel(parent), m_countScanning(0) {
  m_enabled = false;
...
cs8NetworkScanner::~cs8NetworkScanner() { cancel(); }
...
void cs8NetworkScanner::doScan() {
  ...
  foreach (iface, interfaceList) {
    ...
    if (iface.flags() && (QNetworkInterface::IsUp | QNetworkInterface::IsRunning) != 0) {
      qDebug() << "Scanning iface: "
               << " address: " << iface.hardwareAddress() << "flags: " << iface.flags();
      QList<QNetworkAddressEntry> entries;
      foreach (auto entry, iface.addressEntries()) {
        // only scan non local host address and where broadcast address is set
        if (!entry.ip().toString().startsWith("127.") && (!entry.ip().toString().contains(":")) &&
...
int cs8NetworkScanner::columnCount(const QModelIndex & /*index*/) const { return 2; }
...
bool cs8NetworkScanner::enabled() const { return m_enabled; }
...
QTcpSocket *cs8ScannerItem::socket() const { return m_socket; }

bool cs8ScannerItem::occupied() const { return m_occupied; }

void cs8ScannerItem::checkHost(const QString &address) { m_socket->connectToHost(address, 21); }
