...
cs8UserType::cs8UserType(const QString &name, const QString &path, bool autoLoad) {
  m_name = name;
  m_path = path;
  m_autoLoad = autoLoad;
}
...
QString cs8UserType::documentation() const { return m_documentation; }

QString cs8UserType::definition() const {
  qDebug() << "alias: " << m_name << ":" << m_path.split("/").last();
  return QString("%1 %2").arg(m_path.split("/").last(), m_name);
}

void cs8UserType::setDocumentation(const QString doc) { m_documentation = doc; }
