...
val3PrecompilerSettings::val3PrecompilerSettings(QDialog *parent) :
    QDialog(parent),
    ui(new Ui::val3PrecompilerSettings)
{
    ui->setupUi(this);
    ...
    QSettings settings;
    ui->cpUnusedPublicGlobalVar->setChecked (settings.value ("reportUnusedPublicGlobalVariables",true).toBool ());
...
void val3PrecompilerSettings::on_buttonBox_accepted()
{
    close ();
}

void val3PrecompilerSettings::on_buttonBox_rejected()
{
    close();
}
