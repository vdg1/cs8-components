...
QtTelnetPrivate::QtTelnetPrivate(QtTelnet *parent)
    : q(parent), socket(0), notifier(0),
      ...
      curauth(0), nullauth(false), loggedIn(false),
...
void QtTelnetPrivate::setSocket(QTcpSocket *s)
{
    ...
    connected = false;
    if (socket) {
        connect(socket, &QAbstractSocket::connected, this, &QtTelnetPrivate::socketConnected);
        connect(socket, &QAbstractSocket::disconnected, this, &QtTelnetPrivate::socketConnectionClosed);
        connect(socket, &QIODevice::readyRead, this, &QtTelnetPrivate::socketReadyRead);
        connect(socket, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(socketError(QAbstractSocket::SocketError)));
        connect(socket, &QAbstractSocket::stateChanged, this, &QtTelnetPrivate::socketStateChanged);
    }
}
...
void QtTelnet::sendAYT()
{
    sendControl(AreYouThere);
}
