...
void MainWindow::createAPIs(QList<cs8Application *> cs8SourceApps, bool copyInProjectFolder)
{
    ...
    foreach(cs8Application *cs8SourceApp,cs8SourceApps)
    {
        ...
        if (cs8SourceApp->programModel()->publicPrograms().count()>0)
        {
            ...
            foreach(cs8Program *program, cs8SourceApp->programModel ()->programList())
            {


                if ((program->name ().startsWith ("_") && program->isPublic()) || program->name().startsWith("zz"))
                {
                    QString destAppName;
                    ...
                    if (!cs8DestApp->libraryModel ()->contains(cs8SourceApp->name ()))
                    {
                        ...
                        cs8DestApp->libraryModel ()->add (cs8SourceApp->name (),cs8SourceApp->cellProjectFilePath (true),true);
                    }

                    cs8Program *newProgram=cs8DestApp->programModel()->createProgram(program->name());
                    name=program->name ();
            ...
            if (cs8DestApp)
            {
                ...
                cs8Program *initProgram=cs8DestApp->programModel()->createProgram("init");
                ...
                if (cs8DestApp && ioMapApplication.openFromPathName(cs8SourceApp->projectPath()+"/io"+moduleBaseName))
                {
                    ...
                    pth=pth.replace (QDir::fromNativeSeparators(ioMapApplication.cellPath()+"usr/usrapp/"),"Disk://");
                    pth=QDir::fromNativeSeparators (pth);
                    qDebug() << pth;
                    cs8DestApp->libraryModel()->add(ioMapApplication.name(),pth+"/"+ioMapApplication.name()+"/"+ioMapApplication.name()+".pjx" /* ioMapApplication.cellProjectFilePath() */,true);
                ...
                if (copyInProjectFolder)
                {
                    ...
                    QString destDir=cs8SourceApp->projectPath()+"/"+cs8DestApp->name();
                    QDir dir;
                    dir.mkpath(destDir);
                    dir.setPath(sourceDir);
                    foreach (QString fileName, dir.entryList(QStringList(), QDir::Files))
                    {
                        QFile file;
